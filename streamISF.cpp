...
int main(int argc, char **argv)
{
    ...
    for (int bloco = 0; bloco < quantDeBlocos; bloco++)
    {
        ...
        if (label_video == nullptr) {
            fprintf(stderr, "Erro ao alocar memória para label_video\n");
            delete[] video;
            exit(1);
        }
        ...
        while (i < frames && imgFrames <= num_frames)
        {
            ...
            if (imgD == nullptr || border_img == nullptr) {
                fprintf(stderr, "Erro ao alocar memória para imgD ou border_img\n");
