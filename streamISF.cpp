...
int main(int argc, char **argv)
{
    ...
    for (int bloco = 0; bloco < quantDeBlocos; bloco++)
    {
        ...
        // (condicional movida para cálculo de k, antes de entrar no loop)
        if (propagatedMoreLabels == 1) {
            sum = aux + sum;
            if (k > aux2) {
                k = nSVX - sum;
        ...
        // Inicialização dos elementos de 'label_video' como nulo
        for (int j = 0; j < frames; j++) {
            label_video[j] = nullptr;
        }
        ...
        Intersection **label_intersecao = new Intersection*[num_frames - 1];

        for (int j = 0; j < num_frames - 1; j++) {
            label_intersecao[j] = nullptr;
        }
        ...
        for (i = 0; i < (num_frames-1); i++)
        {
            ...
            Image *atual = label_video[i];
            Image *proxima = label_video[i+1];

            label_intersecao[i] = insertIntersection(&atual, &proxima, num_pixels, intersection);
            //gravarArvoresEmArquivo (&proxima, &atual, graph->num_rows, graph->num_cols, frames, imgPorIntersecao, "original.txt", "intersecao.txt", "\n----------------após propagacao", bloco, intersection);
        }
